...
Trelloのような「リスト × タスク」形式のかんばんボードで、個人のタスクを素早く整理できるツールを用意する。まずは最小構成で動くものを作り、実際に使いながら必要な機能を見極めて拡張していく。
...
| 要件 | 内容 | 状態 |
| --- | --- | --- |
| リスト表示 | ボード上に複数のリストを横並びで表示する。タスクが0件のリストも列として表示する。 | 実装済み |
| リスト追加 | ボード右端の「+ リストを追加」から新しいリストを末尾に追加できる。 | 実装済み |
| リスト名編集 | リスト見出しの入力欄を直接編集して名称を変更できる（Enter/フォーカス移動で確定、Escで取消）。 | 実装済み |
| リスト削除 | リスト単位で削除できる。タスクの有無に関わらず確認ダイアログを表示し、リスト内のタスクもあわせて削除する。 | 実装済み |

### 4.2 タスク

| 要件 | 内容 | 状態 |
| --- | --- | --- |
| タスク追加 | フォームからリスト・タイトル・優先度・期限日を指定してタスクを作成する。 | 実装済み |
| タスク編集 | タスクをクリックして開く詳細モーダルで、タイトル・優先度・期限日を編集できる。 | 実装済み |
| タスク削除 | タスク詳細モーダルから削除できる。 | 実装済み |
| タスク移動 | ドラッグ&ドロップで同一リスト内の並べ替え、および他のリスト（空のリストを含む）への移動ができる。 | 実装済み |
| 優先度設定 | タスクごとに優先度を「高・中・低」の3段階で設定し、色（赤・黄・緑）で可視化する。 | 実装済み |
| 期限設定 | タスクごとに期限日を設定できる。期限日を過ぎたタスクは強調表示する。 | 実装済み |

### 4.3 検索・並び替え

| 要件 | 内容 | 状態 |
| --- | --- | --- |
| キーワード検索 | タスクのタイトルを部分一致で検索する。 | 実装済み |
| 優先度による絞り込み | 優先度（高・中・低）でタスクを絞り込む。 | 実装済み |
| リストによる絞り込み | 所属リストIDでタスクを絞り込む。 | 実装済み |
| 並び替え | 優先度順・期限順でリスト内のタスクを並び替える。 | 実装済み |
...
| [データモデル](requirements/data-model.md) | DBのER図（boards / lists / tasks） |
